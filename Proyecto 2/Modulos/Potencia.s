...
.data
    ...
    errorImport:
        .asciz "Error en el Comando De POTENCIAR en la letra A LA"
    ...
    readSuccess:
        .asciz "La operacion POTENCIAR se ha realizado Correctamente\n"
    ...
    erroPotencia:
        .asciz "\n...El programa no considera Potencias NEGATIVAS..."
        lenerroPotencia = .-erroPotencia
...
cargar_referencia:
    ...
    sumar_variables:
        ...
        MOV x4, 1 //Inicializamos la variable que guardara la suma temporal
        ...
        IF:
            ...
            IF_ANIDADO:
                MOV x7, x0 // Carga la base num 
                MOV x6, x3  // Carga el exponente
                B comparar_expo_negativo //Saltamoa a cargar el numero
            ELSE_ANIDADO:
                MOV x7, x0 // Carga la base num 
                MOV x6, x2  // Carga el exponente
                B comparar_expo_negativo //Saltamoa a cargar el numero
        ELSE:
            CMP x3, 0 //Comparamos num6 con el valor de cero
        ...
        error_exponente:
            print 1, erroPotencia, lenerroPotencia //Imprime el mesaje de error por tener una potencia negativa
            read 0, character, 2    // Leer dos caracteres de entrada
            RET                     // Retornar del procedimiento

        comparar_expo_negativo:
            cmp w6, #0            // Comparar w1 con 0
            bmi error_exponente       // Si w1 es menor que 0, saltar a error_exponente

        // Comenzar el bucle de multiplicación
        power_loop:
            cmp w6, #0            // Comparar el exponente con 0
            beq salida         // Si el exponente es 0, saltar al final

            mul x4, x4, x7        // Multiplicar resultado actual (w4) por la base (w7)
            sub w6, w6, #1        // Restar 1 al exponente
            b power_loop          // Repetir hasta que el exponente sea 0
